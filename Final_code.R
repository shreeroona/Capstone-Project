...
server <- function(input, output, session) {
  ...
  output$grid_graph <- renderPlot({
    ...
    vars <- c("connectedness"="black", "spillover"="red")
    ggplot(tbl) + 
      geom_line(aes(x=Date,y=Connectedness,colour="connectedness")) +
      geom_line(aes(x=Date,y=Spillover,colour="spillover")) +
  ...
  output$connected_table <- renderTable({
    conn_table()
  },rownames = TRUE,caption=paste("Connectedness Table"),
  ...
  output$spillover_table <- renderTable({
    spill_table()
  },rownames = TRUE,caption=paste("Returns Spillover Table"),
  caption.placement = getOption("xtable.caption.placement", "top"),
  caption.width = getOption("xtable.caption.width", NULL))
